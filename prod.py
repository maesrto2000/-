...
def handle_callback_query(call):
    if call.data == 'school':
        if init_message == None:
            ...
            user_message = f'Сгенерируй 3 вопросов для школьника по теме {init_message}'
        ...
        if init_message == None:
            ...
            user_message = f'Сгенерируй 3 сложных вопросов для студента по теме {init_message}'
            chat_id = call.message.chat.id
            chat = Chat(id = chat_id, type = 'private')
            message_object = Message(message_id=1, from_user=None, date=None, chat=chat, content_type=None, options={}, json_string=None)
            message_object.text = user_message
        ...
        if init_message == None:
            ...
            user_message = f'Сгенерируй 3 средних по сложности вопросов по теме {init_message}'
            chat_id = call.message.chat.id
            chat = Chat(id = chat_id, type = 'private')
            message_object = Message(message_id=1, from_user=None, date=None, chat=chat, content_type=None, options={}, json_string=None)
            message_object.text = user_message
...
def main(message):
    ...
    try:
        ...
        if user_message == f'Сгенерируй 3 вопросов для школьника по теме {init_message}' or user_message == f'Сгенерируй 3 сложных вопросов для студента по теме {init_message}' or user_message == f'Сгенерируй 3 средних по сложности вопросов по теме {init_message}':
            global waiting 
            global user_answer
            global new
            waiting = 'test'
